...
export function urlObjectToString(o: { [key: string]: any }): string | undefined {
    ...
    // Handle custom conference metadata as simple URL parameters
    const customParams: string[] = [];

    // Support passing via top-level or inside configOverwrite
    const creatorUuid = (o as any).creatorUuid ?? (o as any).configOverwrite?.creatorUuid;
    let cohostsUuids: unknown = (o as any).cohostsUuids ?? (o as any).configOverwrite?.cohostsUuids;

    // Normalize and encode creatorUuid as a JSON string for compatibility with URL JSON parsing
    if (typeof creatorUuid === "string" && creatorUuid.length > 0) {
        customParams.push(`creatorUuid=${encodeURIComponent(JSON.stringify(creatorUuid))}`);
    }

    // Normalize cohostsUuids to a non-empty array of strings
    let cohostsArray: string[] = [];
    if (Array.isArray(cohostsUuids)) {
        cohostsArray = (cohostsUuids as unknown[]).filter((v): v is string => typeof v === "string" && v.length > 0);
    } else if (typeof cohostsUuids === "string" && cohostsUuids.length > 0) {
        cohostsArray = [cohostsUuids];
    }

    if (cohostsArray.length > 0) {
        customParams.push(`cohostsUuids=${encodeURIComponent(JSON.stringify(cohostsArray))}`);
    }

    if (customParams.length) {
        const customParamsString = customParams.join("&");
        if (hash.length) {
            hash += `&${customParamsString}`;
        } else {
            hash = `#${customParamsString}`;
        }
    }
