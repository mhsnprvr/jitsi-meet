...
const GeneralSection = () => {
    ...
    const { disableSelfView } = useSelector((state: IReduxState) => state["features/base/settings"]);

    const { language = DEFAULT_LANGUAGE } = i18next;

    const onSelfViewToggled = useCallback(
        (enabled?: boolean) => dispatch(updateSettings({ disableSelfView: enabled })),
                ...
                <View style={styles.languageButtonContainer as ViewStyle}>
                    <TouchableHighlight onPress={navigateToLanguageSelect}>
                        <View style={styles.languageButton as ViewStyle}>
                            ...
                            <Icon size={24} src={IconArrowRight} />
                        </View>
                    </TouchableHighlight>
                </View>
            </FormRow>
