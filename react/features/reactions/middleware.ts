...
import { setStartReactionsMuted } from "../base/conference/actions.any";
    ...
    switch (action.type) {
                    ...
                    for (let i = 0; i < SOUNDS_THRESHOLDS.length; i++) {
                        dispatch(
                            registerSound(
                                ...
                                { volume: soundsReactionsVolume }
                            )
                        );
                    }
                });
                dispatch(registerSound(RAISE_HAND_SOUND_ID, RAISE_HAND_SOUND_FILE, { volume: soundsReactionsVolume }));
            });
            break;
        ...
        case CONFERENCE_JOIN_IN_PROGRESS: {
            ...
            conference.addCommandListener(
                MUTE_REACTIONS_COMMAND,
                ({ attributes }: { attributes: IMuteCommandAttributes }, id: any) => {
        ...
        case SETTINGS_UPDATED: {
            ...
            // Update volume for reaction sounds if volume setting changed
            if (action.settings.soundsReactionsVolume !== undefined) {
                const state = getState();
                ...
                // Update raise hand sound volume
                const raiseHandSound = sounds.get(RAISE_HAND_SOUND_ID);
                if (
                    raiseHandSound &&
