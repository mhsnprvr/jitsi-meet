...
import { BackHandler, Platform, StyleProp, Text, TextStyle, View, ViewStyle } from "react-native";
...
const Prejoin: React.FC<IPrejoinProps> = ({ navigation }: IPrejoinProps) => {
    ...
    const aspectRatio = useSelector((state: IReduxState) => state["features/base/responsive-ui"]?.aspectRatio);
    ...
    const isDisplayNameVisible = useSelector(
        (state: IReduxState) => !getFeatureFlag(state, PREJOIN_PAGE_HIDE_DISPLAY_NAME, false)
    );
    ...
    const headerLeft = () => {
        if (Platform.OS === "ios") {
            return <HeaderNavigationButton label={t("dialog.close")} onPress={goBack} />;
        }
    ...
    return (
        <JitsiScreen addBottomPadding={false} safeAreaInsets={["right"]} style={contentWrapperStyles}>
                    ...
                    <View style={styles.conferenceInfo as StyleProp<ViewStyle>}>
                        {roomNameEnabled && (
                            <View style={styles.displayRoomNameBackdrop as StyleProp<TextStyle>}>
                                <Text numberOfLines={1} style={styles.preJoinRoomName as StyleProp<TextStyle>}>
                                    {roomName}
                                </Text>
                            </View>
                        )}
            ...
            <View style={contentContainerStyles as ViewStyle}>
                <View style={styles.toolboxContainer as ViewStyle}>
                    ...
                    {showHangUp && knocking && <HangupButton styles={styles.buttonStylesBorderless} />}
                </View>
